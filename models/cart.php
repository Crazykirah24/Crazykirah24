...
class cartsmodel {
    ...
    public function loadFromDb($user_id) {
        try {
            ...
            foreach ($results as &$item) {
                $item['images'] = !empty($item['image_urls']) ? json_decode($item['image_urls'], true) : [];
        ...
        try {
            foreach ($panier_session as $item) {
                ...
                if ($existing) {
                    ...
                    $newQty = $quantite; // Remplacer au lieu d'additionner
                    $sqlUpdate = "
                        UPDATE panier 
                        SET quantite = :quantite, 
                            prix_original = :prix_original, 
    ...
    public function remove($produit_id) {
        ...
        try {
            ...
            if (!$result) {
                throw new Exception("Aucun produit trouvé à supprimer");
            }
            return true;
        } catch (PDOException $e) {
